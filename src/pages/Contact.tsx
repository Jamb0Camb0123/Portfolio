...
const Contact: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl w-full flex flex-col md:flex-row gap-x-12 items-center justify-center px-4 md:px-0 -mt-12">
          ...
          {!sent ? (
            <form ref={form} onSubmit={sendEmail} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" id="email" name="user_email" required className="w-full border border-gray-300 rounded px-3 py-2" />
              </div>

              <div>
                <label htmlFor="message" className="block font-medium mb-1">Message:</label>
                <textarea id="message" name="message" rows={5} required className="w-full border border-gray-300 rounded px-3 py-2" />
              </div>
              ...
              <button type="submit" className="w-full mt-4 bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded transition cursor-pointer">
                Send
              </button>
            </form>
          ) : (
